...
def submit_usuario():
    ...
    print(nome,cpf,ende,cidade,habilitacao,cel,email)
...
def usuario():
    ...
    #botão interativo
#    ttk.Label(usuario, text = "selecione a cidade :", 
#    		font = ("Times New Roman", 10)).grid(column = 0, 
#    		row = 5, padx = 10, pady = 25) 

    # Combobox creation 
    n = tk.StringVar() 
    monthchoosen = ttk.Combobox(usuario, width = 17, textvariable = n, values=cidades) 

    # Adding combobox drop down list 
    # monthchoosen['values'] = (' PL', 
    # 						' SC', 
    # 						' RS') 

    monthchoosen.grid(column = 1, row = 3) 
    
    usuarios.append(monthchoosen) #util
...
def veiculo():
    ...
    n = tk.StringVar() 
    monthchoosen = ttk.Combobox(veiculo, width =17, textvariable = n) 
#    monthchoosen['values'] = ('usuario1', 
 #   						'usuario2', 
  #  						'usuario3')
    ...
    veiculos.append(monthchoosen) #util
    monthchoosen.current() 
...
def cidade():
    ...
    # cidades.append(f'{txtnome.get()}, {txtuf.get()}')

    btnSubmit= Button(cidade,text="submit", command=lambda: submit_cidade(txtnome, txtuf)).grid(row=6,column=0,padx=5,pady=5)
